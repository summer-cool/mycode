window.onload = function() {
    ...
    var gameMonitor = {
        ...
        isMobile: function() {
                gameMonitor.im.loadImage(['imgs/0.png']);
                gameMonitor.im.loadImage(['imgs/1.png']);
                gameMonitor.im.loadImage(['imgs/2.png']);
                gameMonitor.im.loadImage(['imgs/3.png']);
    ...
    websocket.onmessage = function(event) {
        ...
        } else if(strToJson(event.data).type == "updatefriend"){ /*刷新好友列表和分数*/
            AlreadyPlay(friendUrl);     
        }else if(strToJson(event.data).type == "updatescore"){
            getImage(headurl, openid);
        }
    }
};